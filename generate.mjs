...
const MESH = ({ grid, size, bg }) => {
  ...
  // Scale strictly proportional to width; center vertically.
  const offY = (H - W) / 2;
  ctx.drawImage(window._mid, 0, offY, W, W);
`;
};
...
<style>
  ...
  .controls { position:fixed; top:16px; left:16px; z-index:3; display:flex; gap:6px; align-items:center; }
  .controls > * { font:inherit; letter-spacing:.12em; text-transform:uppercase; font-size:10px; font-weight:600;
                   height:28px; border-radius:999px; border:0; cursor:pointer; }
  .controls a, .controls button { color:#fff; background:#111; padding:0 12px; text-decoration:none; display:inline-flex; align-items:center; }
  .controls a:hover, .controls button:hover { background:#000; }
  .controls select { color:#111; background:#fff; padding:0 10px; cursor:pointer; -webkit-appearance:none; appearance:none; padding-right:22px;
                     background-image:url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='8' height='6' viewBox='0 0 8 6'><path d='M0 0l4 6 4-6z' fill='%23111'/></svg>");
                     background-repeat:no-repeat; background-position:right 8px center; }
  button.toggle.on { background:#fff; color:#111; }
  ...
  .peek .chip { position:absolute; top:16px; left:16px; letter-spacing:.12em; text-transform:uppercase;
                font-size:10px; font-weight:600; color:#111; background:#fff; padding:6px 10px; border-radius:999px; }
</style>
</head>
<body>
<canvas id="c"></canvas>
